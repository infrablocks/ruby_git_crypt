...
describe RubyGitCrypt::Options::Types do
  describe '.flag' do
    it 'builds a flag type' do
      expect(described_class.flag(
               '-flag', RubyGitCrypt::Options::Values.boolean(true)
             ))
        .to(eq(RubyGitCrypt::Options::Types::Flag.new(
                 '-flag', RubyGitCrypt::Options::Values.boolean(true)
               )))
    end
  end
  ...
  describe '.standard' do
    it 'builds a standard type' do
      expect(described_class.standard(
               '-opt',
               RubyGitCrypt::Options::Values.string('/some/path')
             ))
        .to(eq(RubyGitCrypt::Options::Types::Standard.new(
                 '-opt',
                 RubyGitCrypt::Options::Values.string('/some/path')
               )))
    end
